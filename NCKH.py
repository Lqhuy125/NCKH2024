...
import math
import serial
cap = cv2.VideoCapture("Anh/drive.mp4")
#ser = serial.Serial("COM8", 9600)   ## Mở cổng kết với ardiono của bạn
    ...
    if ret:
        ...
        for line in lines:
            ...
            try:
                ...
                # them phan re phai nhieu hay re phai it
                if angle < -45:
                    huong_re = "A"
                    cv2.putText(frame, "LEFT", (15, 35), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
                elif angle > -45 and angle < -20:
                    huong_re = "B"
                    cv2.putText(frame, "LEFT-IT", (15, 35), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
                elif angle > -20 and angle < 20:
                    huong_re = "C"
                    cv2.putText(frame, "DITHANG", (600,35),cv2.FONT_HERSHEY_SIMPLEX,1 , (255,0,0), 2, cv2.LINE_AA )
                elif angle > 20 and angle < 45:
                    huong_re = "D"
                    cv2.putText(frame, "RIGHT-IT", (1150, 35), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
                elif angle > 45:
                    huong_re = "E"
                    cv2.putText(frame, "RIGHT", (1150, 35), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
               # if ser.isOpen():
                #    ser.write(str(huong_re).encode())
            except NameError:
                continue
